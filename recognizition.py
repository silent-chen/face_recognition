...
def face_validation(facesavename_tmp):
    ...
    shape = predictor(unknown_image, rects[0])
    ...
    results = face_recognition.compare_faces(known_faces, unknown_face_encoding, 0.55)
...
#        csvfile.close
def addface(filepath,unknown_face_name):
    _init_()
    # image_name = url.split(';')
    # arr = []
    # item = []
    ...
    facesavename = align_faces(filepath)
    for i in range(len(facesavename)):
        # Get the face encodings for each face in eac image file
        # Since there could be more than one face in each image, it returns a list of encodings.
        # But since I know each image only has one face, I only care about the first encoding in each image, so I grab index 0.
        # face_recognition_model = face_recognition_models.face_recognition_model_location()
        # face_encoder = dlib.face_recognition_model_v1(face_recognition_model)
        ...
        # win.clear_overlay()
        # win.set_image(unknown_image)
        # dlib.hit_enter_to_continue()    facerec = dlib.face_recognition_model_v1(sys.path[0]+"/models/dlib_face_recognition_resnet_model_v1.dat")
        #
        unknown_image = face_recognition.load_image_file(facesavename[i])
        cnn_face_detector = dlib.cnn_face_detection_model_v1(sys.path[0] + "/models/mmod_human_face_detector.dat")
        gray = cv2.cvtColor(unknown_image, cv2.COLOR_BGR2GRAY)
        dets = cnn_face_detector(gray, 1)
        rects = dlib.rectangles()
